...
public class Map
{
	/// <summary>For testing if >= 0 then the specified seed will be used for terrain generation rather than the normal algorithms</summary>
	private const int TestingTerrainSeed = -1;

	/// <summary>For testing if >= 0 then the specified seed will be used for locations rather than the normal algorithms</summary>
	private const int TestingLocationsSeed = -1;
	...
	public Map()
	{
		TerrainSeed = Math.Abs(Environment.TickCount);

		// This selects one of the premade seeds that was found by trial and error
		{
			var seedSelectorRandom = new System.Random(TerrainSeed);
			TerrainSeed = GoodSeeds[seedSelectorRandom.Next(GoodSeeds.Length)];
		}

		if(TestingTerrainSeed >= 0)
		{
			TerrainSeed = TestingTerrainSeed;
		}

		random = new System.Random(TerrainSeed);
		...
		if (TestingLocationsSeed < 0)
		{
			LocationsSeed = Math.Abs(Environment.TickCount);
		}
		random = new System.Random(LocationsSeed);
		...
		var allyHouse = playersPossibleNodes[random.Next(playersPossibleNodes.Count)];
		initialAllyLocations = CalculateUnitLocations(new List<int>() { allyHouse }, 10, 8 + 16 + 24);

		initialEnemyHouseLocations = CalculateEnemyHouseLocations(2, Math.Min(6, gridIndexToPossibleEnemyNodes.Count), gridIndexToPossibleEnemyNodes);
		initialEnemyLocations = CalculateUnitLocations(initialEnemyHouseLocations, 20, 8 + 16 + 24);
